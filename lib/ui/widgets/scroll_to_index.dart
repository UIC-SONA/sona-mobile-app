...
abstract class AutoScrollController implements ScrollController {
  factory AutoScrollController({double initialScrollOffset = 0.0, bool keepScrollOffset = true, double? suggestedRowHeight, ViewportBoundaryGetter viewportBoundaryGetter = defaultViewportBoundaryGetter, Axis? axis, String? debugLabel, AutoScrollController? copyTagsFrom}) {
    return SimpleAutoScrollController(initialScrollOffset: initialScrollOffset, keepScrollOffset: keepScrollOffset, suggestedRowHeight: suggestedRowHeight, viewportBoundaryGetter: viewportBoundaryGetter, beginGetter: axis == Axis.horizontal ? (r) => r.left : (r) => r.top, endGetter: axis == Axis.horizontal ? (r) => r.right : (r) => r.bottom, copyTagsFrom: copyTagsFrom, debugLabel: debugLabel);
  }
...
class SimpleAutoScrollController extends ScrollController with AutoScrollControllerMixin {
  ...
  SimpleAutoScrollController({super.initialScrollOffset, super.keepScrollOffset, this.suggestedRowHeight, this.viewportBoundaryGetter = defaultViewportBoundaryGetter, required this.beginGetter, required this.endGetter, AutoScrollController? copyTagsFrom, super.debugLabel}) {
...
class PageAutoScrollController extends PageController with AutoScrollControllerMixin {
  ...
  PageAutoScrollController({super.initialPage, super.keepPage, super.viewportFraction, this.suggestedRowHeight, this.viewportBoundaryGetter = defaultViewportBoundaryGetter, AutoScrollController? copyTagsFrom, String? debugLabel}) {
    if (copyTagsFrom != null) tagMap.addAll(copyTagsFrom.tagMap);
  }
}
...
class AutoScrollTag extends StatefulWidget {
  ...
  const AutoScrollTag({required Key key, required this.controller, required this.index, this.child, this.builder, this.color, this.highlightColor, this.disabled = false})
...
class AutoScrollTagState<W extends AutoScrollTag> extends State<W> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return widget.builder?.call(context, animation) ?? buildHighlightTransition(context: context, highlight: animation, child: widget.child!, background: widget.color, highlightColor: widget.highlightColor);
  }
...
Widget buildHighlightTransition({required BuildContext context, required Animation<double> highlight, required Widget child, Color? background, Color? highlightColor}) {
