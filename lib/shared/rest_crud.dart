...
Future<List<T>> _list<T>(Uri uri, String path, Map<String, String> headers, http.Client? client, [String? search]) async {
  final response = await request(
    uri.replace(path: path, queryParameters: {'search': search}),
    client: client,
...
abstract class RestListable<T> implements WebResource, Listable<T> {
  @override
  Future<List<T>> list([String? search]) async => _list<T>(uri, path, headers, client, search);
}

abstract class RestPaginable<T> implements WebResource, Paginable<T> {
  @override
  Future<Page<T>> page([PageQuery? query]) async => _page<T>(uri, path, headers, client, query);
}

abstract interface class RestFindable<T, ID> implements WebResource, Findable<T, ID> {
  @override
  Future<T> find(ID id) async => _find<T, ID>(uri, path, headers, client, id);
}

abstract class RestCountable implements WebResource, Countable {
  @override
  Future<int> count() async => _count(uri, path, headers, client);
}

abstract class RestExistable<ID> implements WebResource, Existable<ID> {
  @override
  Future<bool> exists(ID id) async => _exists<ID>(uri, path, headers, client, id);
}

abstract class RestCreatable<T, D> implements WebResource, Creatable<T, D> {
  @override
  Future<T> create(D dto) async => _create<T, D>(uri, path, headers, client, dto);
}

abstract class RestUpdatable<T, D, ID> implements WebResource, Updatable<T, D, ID> {
  @override
  Future<T> update(ID id, D dto) async => _update<T, D, ID>(uri, path, headers, client, id, dto);
}

abstract class ApiDeletable<ID> implements WebResource, Deletable<ID> {
  @override
  Future<void> delete(ID id) async => _delete<ID>(uri, path, headers, client, id);
}
